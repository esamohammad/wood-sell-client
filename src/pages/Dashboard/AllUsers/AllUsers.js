...
const AllUsers = () => {
    ...
    const { data: users = [], isLoading, refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://wood-sell-server.vercel.app/users');
    ...
    const handleMakeAdmin = id => {
        fetch(`https://wood-sell-server.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
    ...
    const handleDeleteUser = p => {
        fetch(`https://wood-sell-server.vercel.app/users/${p._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
